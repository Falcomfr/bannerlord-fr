...
<?php if(!empty($_GET['page'] == "populaire")) {} else {
    echo '<input type="text" id="keywords" onkeyup="searchFilter()"/> ';
    echo '<select id="sortBy" onchange="searchFilter()">';
        echo '<option value="tous">Tout afficher</option>';
        echo '<option value="2">News</option>';
        echo '<option value="3">Articles</option>';
        echo '<option value="4">Patchnote</option>';
        echo '<option value="5">Vidéos</option>';
    echo '</select>';
}
...
<div class="post-wrapper">
    ...
    $query = $db->query("SELECT * FROM actu ORDER BY vues DESC LIMIT $limit");
